...
## 2026-07-15 — 7단계: 마이페이지 (히스토리/축별 평균 추이)

브리프 섹션 8의 MVP 구현 우선순위 7개 항목이 이번 단계로 모두 완료됨.

### 완료 내용

1. **히스토리 조회** (`src/lib/roundsApi.js`) — `getUserRoundHistory`: 유저의
   **전체 티어**에 걸친 완료 라운드를 시간순(오래된 순)으로 조회, 주제명 +
   채점 결과 임베딩. 채점이 실패해 `judgments`가 비어있는 라운드도 그대로
   포함해(히스토리에서는 "미도전" 별점으로 구분 표시) 데이터 누락 없이 보여줌.

2. **통계 로직** (`src/lib/historyStats.js`)
   - `buildTrendData`: 채점 완료된 라운드만 추출해 시간순 인덱스를 부여,
     라인 차트용 데이터로 변환(축별 평균 추이)
   - `calcOverallAverages`: 3축 전체 평균 + 완료 라운드 수 계산

3. **마이페이지 화면** (`src/pages/MyPage.jsx`)
   - 상단: 닉네임/티어/경험치
   - 통계 카드: 완료 라운드 수 + 3축 전체 평균 + 종합 별점
   - `recharts`의 `LineChart`로 3축(논증 타당성/논점 대응력/설득력 전개)
     점수 추이를 라운드 순서에 따라 시각화(추이를 보려면 채점 완료 라운드
     2개 이상 필요 — 1개 이하일 때는 안내 문구로 대체)
   - 라운드 히스토리 목록(최신순): 주제명, 티어, 상대 페르소나, 완료일,
     3축 점수, 별점

4. **홈 화면 연동** (`src/pages/HomePage.jsx`) — 헤더에 "마이페이지" 링크 추가,
   `App.jsx`에 `/mypage` 라우트 추가.

### 테스트 결과

- **코드 레벨 테스트** (`npm test`): 105개 항목 전부 통과 (기존 95개 + 히스토리
  통계 로직 10개 추가) — 채점 실패 라운드 제외 처리, 시간순 인덱스, 평균 계산,
  빈 히스토리 처리 등 검증
- **DB 실동작 검증** (로컬 PostgreSQL): 브론즈 2라운드(1개는 채점 실패
  시나리오) + 실버 1라운드(승급 후)를 생성해, `getUserRoundHistory`와 동등한
  조인 조회로 **전체 티어에 걸쳐 시간순** 정확히 조회됨을 확인. 채점 실패
  라운드의 점수 필드가 NULL로 정확히 조회됨을 확인
- **E2E 브라우저 테스트** (Playwright + Chromium, Supabase REST 스텁. 테스트 후
  `.env`와 임시 스크립트 삭제):
  - 마이페이지 진입 시 티어/경험치/완료 라운드 수/3축 평균이 정확히 표시됨
    (브론즈 60/65/70점 + 실버 85/88/90점 라운드로 평균 검증)
  - 라인 차트(recharts SVG) 정상 렌더링 확인
  - 채점 실패 라운드가 "미도전" 별점으로 올바르게 구분 표시됨을 확인
  - 히스토리가 최신순으로 정렬되어 표시됨을 확인
  - 홈 화면의 "마이페이지" 링크 클릭 시 `/mypage`로 정상 이동함을 확인
- **빌드/린트**: `npm run build`, `npm run lint`(oxlint) 통과 (번들 크기 경고
  지속 — 현재 약 1MB. 기능상 문제는 없으나, MVP 7단계가 모두 끝났으므로
  다음 라운드에서 코드 스플리팅(`React.lazy` 등)을 고려할 시점)

### 사용자가 직접 확인해야 할 항목

1. 실제로 여러 라운드를 플레이한 뒤 마이페이지의 축별 평균 추이 그래프가
   실력 향상을 잘 보여주는지 확인해주세요.
2. 채점이 실패한 라운드가 실제로 발생했을 때(네트워크 오류 등) 히스토리에서
   "미도전"으로 표시되는 것이 직관적으로 이해되는지 확인해주세요(다른 문구가
   더 명확하다면 "채점 실패"처럼 별도 문구로 바꿀 수 있습니다).
3. 여러 티어에 걸친 히스토리가 쌓였을 때 목록이 너무 길어지면 페이지네이션이
   필요할 수 있습니다 — 현재는 전체를 한 번에 불러옵니다.

### 브리프 섹션 8 MVP 구현 우선순위 — 전체 완료

1. ✅ DB 스키마 + 시드 데이터 (1단계)
2. ✅ 온보딩 화면 (2단계)
3. ✅ 홈 화면 (3단계)
4. ✅ 대화(라운드) 화면 — AI 논객 API 연동 (4단계)
5. ✅ 채점 API + 결과 화면 (5단계)
6. ✅ 경험치/티어 승급 로직 (6단계)
7. ✅ 마이페이지 (7단계)
...
MVP 7단계 구현이 모두 끝났습니다. 브리프 섹션 9(신뢰성 검증 — 채점 프롬프트
반복 검증, 데이터형 페르소나 통계 환각 여부, AI 밸런스 조정)는 프로토타입이
실제로 플레이된 뒤 진행해야 하는 항목으로, 사용자의 실제 플레이 피드백을
기다립니다. Supabase 프로젝트 연결과 API 키 설정 후 실제 플레이 테스트를
진행해주세요.
